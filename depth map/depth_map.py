...
if __name__ == "__main__":
    ...
    # )
    # parser.add_argument(
    #     "--left_source", type=str, required=True, help="Left video or v4l2 device name"
    # )
    # parser.add_argument(
    #     "--right_source",
    #     type=str,
    ...
    # )
    # parser.add_argument(
    #     "--is_real_time", type=int, required=True, help="Is it camera stream or video"
    # )
    ...
    if (
        not cap_left.isOpened() and not cap_right.isOpened()
    ...
    while True:  # Loop until 'q' pressed or stream ends
        ...
        if cv2.waitKey(1) & 0xFF == ord(
            "q"
        ):  # Get key to stop stream. Press q for exit
            break
